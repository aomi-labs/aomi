  ...
  JSON.stringify(
    {
      ...
      backendRuntimeRevision: required(
        "AOMI_STABILITY_BACKEND_RUNTIME_REVISION",
      ),
      frontendRuntimeRevision: required(
        "AOMI_STABILITY_FRONTEND_RUNTIME_REVISION",
      ),
      backendSourceRevision: revision(backendRoot),
      frontendRunnerRevision: revision(resolve(import.meta.dirname, "..")),
      databaseMigrationDigest: required(
        "AOMI_STABILITY_DATABASE_MIGRATION_DIGEST",
      ),
      ...
      commitOrigin: commitOrigin.origin,
      executionRpc: rpcOrigin.origin,
      chainId: 8453,
      sessionId,
      commitId,
      expectedHash,
...
const issuer = (
  await readFile(join(backendRoot, "aomi/bin/api-server/src/auth.rs"), "utf8")
).match(
...
const agentClient = new AomiClient({
  ...
  oauth: async ({ resource, scopes }) => {
    const { bearer, expiresAt } = await mintAgentApiBearer(userId, {
      ...
      resource,
      client_id: "commit-stability-readback",
      auth_source: "oauth",
      principal_class: "user",
      grant_id: `commit-stability-readback-${runId}`,
    });
    return {
      accessToken: bearer,
...
const chain = createPublicClient({
  chain: base,
  transport: http(rpcOrigin.href),
});
const timeline: Record<string, unknown>[] = [];
const event = (phase: string, detail: Record<string, unknown> = {}) =>
  timeline.push({
    phase,
...
try {
  ...
  assert.equal(
    view.state,
    "confirmed",
    "cold SDK readback must remain confirmed",
  );
  assert.equal(view.transaction_id?.toLowerCase(), expectedHash);
  assert.ok(
    view.wallet_attempt?.attempt_id,
    "durable wallet attempt must survive reload",
  );
  event("cold_commit_readback", {
    state: view.state,
    version: view.version,
    attemptId: view.wallet_attempt.attempt_id,
  });

  const receipt = await chain.getTransactionReceipt({
    hash: expectedHash as `0x${string}`,
  });
  const transaction = await chain.getTransaction({
    hash: expectedHash as `0x${string}`,
  });
  ...
  const callbackEvents: TurnEvent[] = [];
  const operationId = view.batch?.batch_id ?? view.commit_id;
  let callback = callbackReadback(callbackEvents, operationId);
  while (performance.now() - start < 90_000) {
    const page = await agentClient.agent.poll(sessionId, {
      cursor,
      waitMs: 10_000,
    });
    for (const item of page.events) {
      ...
      callbackEvents.push(item);
    }
    cursor = page.cursor;
    callback = callbackReadback(callbackEvents, operationId);
    if (
      callback.ready ||
  ...
  result.callbackTurnId = `broadcast-terminal:${operationId}`;
  result.callbackState = callback.state ?? "pending_after_90s";
  result.callbackAnswerReady = callback.ready;
  result.agentEventCount = seen.size;
  result.status = callback.ready ? "PASS" : "BLOCKED";
} catch (error) {
  const e = error as { name?: string; message?: string };
  result.status = "FAIL";
  result.observed = `${e.name ?? "Error"}: ${String(e.message ?? "")
