...
## 5. Caller authentication and App selection

Use guest access for evaluation or an exact-resource OAuth grant for account-owned access. Browser embeds use the widget's origin-bound session. Keep authentication separate from the App selector and wallet approval.

A raw Agent request uses `Authorization: Bearer`, `Content-Type: application/json`, and an `Idempotency-Key`. Its JSON body carries `sessionId`, `message`, and, for Direct routing, `mode: "direct"` plus the App selector. Do not send an App key from browser code.
...
sequenceDiagram
    ...
    Frontend->>Aomi: POST /v1/agent/chat<br/>Bearer + Idempotency-Key<br/>sessionId, message, mode, app
