      ...
      <div className="h-screen overflow-y-auto">
        <div className="bg-aomi-bg text-aomi-fg min-h-full font-sans">
          <div className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-8">
            <Link
              href="/"
              className="text-aomi-muted hover:text-aomi-fg flex w-fit items-center gap-1.5 text-[13px] transition-colors"
            >
              <ArrowLeft size={14} />
              Back to chat
            </Link>
...
function ItemizedContent({
  ...
  const visibleApps = month.apps.filter(
    (a) => appFilter === "all" || a.id === appFilter,
  );

  const computeApps = visibleApps.filter(
    (a) =>
      (showModels && a.model.byModel.length > 0) ||
      (showTools && a.tool !== null),
  );
  ...
  const outcomePairs = showOnchain
    ? visibleApps.flatMap((a) =>
        (a.outcome?.items ?? []).map((item) => ({ app: a, item })),
      )
    : [];
  ...
  if (!showSectionA && !showSectionB) {
    return (
      <div className="border-aomi-border bg-aomi-bg/40 text-aomi-muted rounded-xl border px-4 py-10 text-center text-[13px]">
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="border-aomi-border bg-aomi-bg/40 flex items-center justify-between rounded-xl border px-4 py-3.5">
        ...
        <span className="font-mono text-base font-semibold tabular-nums">
          {usd(
            filtersActive ? sectionATotal + sectionBTotal : summary.totalUsd,
          )}
        </span>
      </div>
      ...
      {!filtersActive && (
        <div className="border-aomi-border bg-aomi-bg/40 flex flex-col gap-2.5 rounded-xl border p-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="border-aomi-border flex flex-col gap-1 border-l pl-3">
                {appRecipients.map((a) => (
                  <div
                    key={a.name}
                    className="text-aomi-muted flex items-center justify-between"
                  >
                    <span>{a.name}</span>
                    <span className="text-aomi-fg font-mono tabular-nums">
                      {usd(a.amount)}
                    </span>
                  </div>
                ))}
          ...
          <span className="text-aomi-muted text-[11px]">
            These tool and outcome fees also appear on the apps&apos; own
            builder statements as Aomi&apos;s take.
          </span>
        </div>
      )}
    </div>
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="border-aomi-border text-aomi-fg hover:bg-aomi-surface-2/60 flex items-center gap-2 rounded-full border px-3.5 py-[7px] text-xs font-medium transition-colors"
      >
        {current?.label}
        <ChevronDown
          size={12}
            ...
            {options.map((o) => {
              ...
              return (
                <button
                  ...
                  className={`hover:bg-aomi-hover flex min-h-8 w-full items-center justify-between gap-3 rounded-lg px-2.5 text-left text-[13px] transition-colors ${
                    selected
                      ? "bg-aomi-accent-subtle text-aomi-fg font-medium"
                      : "text-aomi-muted"
                  }`}
                >
                  {o.label}
