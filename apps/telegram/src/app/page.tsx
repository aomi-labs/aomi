...
export default function Home() {
  ...
  if (permission.status === "done") message = "Permission updated. Return to Telegram.";
    ...
    <main className="wallet-page">
      <section className="wallet-control" aria-live="polite">
        ...
        {launch.status !== "error" && account.status !== "ready" && (
          <button className="para-button" type="button" onClick={() => openModal()}>
            {para.embedded.isConnected ? "Open Para" : "Continue with Para"}
          </button>
        )}
        {permission.status === "ready" && (
          <button className="para-button" type="button" onClick={permission.sign}>
            Sign permission
          </button>
        )}
      </section>
